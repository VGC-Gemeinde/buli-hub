...
- **`dev` is where work happens; `main` is the release branch.** Default to `dev` — commit there freely, push as often as you like, and every push deploys to staging. Production only ever changes by promoting `dev` into `main`, always through a pull request. A **`pre-push` hook rejects direct pushes to `main`** (`.githooks/pre-push`, enabled by `git config core.hooksPath .githooks` — run once per clone). It is a client-side guard, not GitHub branch protection: server-side protection needs a paid plan for private repos and this org is on Free, so it stops accidents but not intent. Deliberate override: `git push --no-verify`.

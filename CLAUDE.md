...
- **UI copy: no em-dashes.** Every user-visible German string — headings, body copy, hints, empty states, button labels, error messages, `title` tooltips — is written in plain, natural German without `—`. Split the sentence with a full stop, or use a comma or colon, whichever a person would actually write. „Der Spieltag ist vorbei. Nachtragen ist noch möglich." not „Der Spieltag ist vorbei — nachtragen ist noch möglich." Two things this does *not* cover: `—` as the empty-value placeholder in a table cell or chip (an established glyph here, see `playerName()` fallbacks and `ddMM`), and code comments, which keep the existing house style. Applies to every feature, including copy carried over from a design hand-off.
